...
spark.createDataFrame(clean_cols(visuals_df).astype(str)).write.mode("overwrite").saveAsTable(f"{CATALOG}.{SCHEMA}.{_t('pbi_visuals')}")
print(f"✓ {CATALOG}.{SCHEMA}.{_t('pbi_visuals')} ({len(visuals_df)} filas)")

spark.createDataFrame(clean_cols(fields_df).astype(str)).write.mode("overwrite").saveAsTable(f"{CATALOG}.{SCHEMA}.{_t('pbi_visual_fields')}")
print(f"✓ {CATALOG}.{SCHEMA}.{_t('pbi_visual_fields')} ({len(fields_df)} filas)")
...
if not styles_df.empty:
    spark.createDataFrame(styles_df.astype(str)).write.mode("overwrite").saveAsTable(f"{CATALOG}.{SCHEMA}.{_t('pbi_styles')}")
    print(f"✓ {CATALOG}.{SCHEMA}.{_t('pbi_styles')} ({len(styles_df)} filas)")
else:
