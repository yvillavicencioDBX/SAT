...
pbi_filters = spark.sql(f"SELECT * FROM {CATALOG}.{SCHEMA}.{_t('pbi_page_filters')}").toPandas()
...
spark.createDataFrame(filters_applied_df.astype(str)).write.mode("overwrite").option("overwriteSchema", "true").saveAsTable(f"{CATALOG}.{SCHEMA}.{_t('pbi_dashboard_filters')}")
print(f"✓ {CATALOG}.{SCHEMA}.{_t('pbi_dashboard_filters')} ({len(filters_applied_df)} filas)")
