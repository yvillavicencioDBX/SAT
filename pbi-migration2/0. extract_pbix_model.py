...
dbutils.widgets.text("catalog", "migracion_pbix", "Catalogo destino (artefactos)")
dbutils.widgets.text("schema", "couch", "Schema destino (artefactos)")
...
# Parsear data_locations igual que notebook 1
DATA_LOCATIONS = []
if DATA_LOCATIONS_RAW:
    for loc in DATA_LOCATIONS_RAW.split(','):
        loc = loc.strip()
        if not loc or '.' not in loc:
            print(f"  ⚠ data_locations '{loc}' no es 'catalog.schema', salteando")
            continue
        c, s = loc.split('.', 1)
...
spark.createDataFrame(clean_cols(combined_df).astype(str)).write.mode("overwrite").saveAsTable(f"{CATALOG}.{SCHEMA}.{_t('pbi_measures')}")
print(f"OK {CATALOG}.{SCHEMA}.{_t('pbi_measures')}")

spark.createDataFrame(clean_cols(context_df).astype(str)).write.mode("overwrite").saveAsTable(f"{CATALOG}.{SCHEMA}.{_t('pbi_context_filters')}")
print(f"OK {CATALOG}.{SCHEMA}.{_t('pbi_context_filters')}")

spark.createDataFrame(clean_cols(page_filters_df).astype(str)).write.mode("overwrite").saveAsTable(f"{CATALOG}.{SCHEMA}.{_t('pbi_page_filters')}")
print(f"OK {CATALOG}.{SCHEMA}.{_t('pbi_page_filters')}")

if not rels_df.empty:
    spark.createDataFrame(clean_cols(rels_df).astype(str)).write.mode("overwrite").saveAsTable(f"{CATALOG}.{SCHEMA}.{_t('pbi_relationships')}")
    print(f"OK {CATALOG}.{SCHEMA}.{_t('pbi_relationships')}")

if not calc_cols.empty:
    spark.createDataFrame(clean_cols(calc_cols).astype(str)).write.mode("overwrite").saveAsTable(f"{CATALOG}.{SCHEMA}.{_t('pbi_calculated_columns')}")
    print(f"OK {CATALOG}.{SCHEMA}.{_t('pbi_calculated_columns')}")
...
# Itera DATA_LOCATIONS (catalog.schema) en lugar del destino, para no listar tablas erróneas
for c_loc, s_loc in DATA_LOCATIONS:
    try:
        tables_in_catalog = spark.sql(f"SHOW TABLES IN {c_loc}.{s_loc}").collect()
        table_names = [r.tableName for r in tables_in_catalog]
        for tname in table_names:
            if tname.startswith("pbi_") or tname.startswith("mv_") or tname.startswith("v_dashboard_") or tname.startswith("lod_"):
                continue
            full = f"{c_loc}.{s_loc}.{tname}"
            try:
                cols = spark.sql(f"DESCRIBE {full}").collect()
                col_names = [r.col_name for r in cols if not r.col_name.startswith('#')]
                existing_tables[full] = col_names
            except Exception as desc_err:
                print(f"  SKIP {tname}: {str(desc_err)[:100]}")
        print(f"Tablas de datos en {c_loc}.{s_loc}:")
        for full, cols in existing_tables.items():
            if full.startswith(f"{c_loc}.{s_loc}."):
                print(f"  {full.split('.')[-1]}: {len(cols)} columnas")
    except Exception as e:
        print(f"Error listando tablas en {c_loc}.{s_loc}: {e}")
